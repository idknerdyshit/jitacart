...
use crate::{db::Tx, errors::ApiError, extract::CurrentList, state::AppState};
...
pub(super) async fn add_items(
    ...
    let cur_count: i64 = sqlx::query_scalar("SELECT count(*) FROM list_items WHERE list_id = $1")
        .bind(list_id)
        .fetch_one(&mut **conn)
        .await?;
...
pub(super) async fn patch_item(
    ...
    recompute_estimates(&state, &tx, list_id, updated_after).await?;
    let detail = load_list_detail(&state, &tx, list_id, cur.user_id, cur.role).await?;
    Ok(Json(detail))
}
